...
namespace DMST {
    ...
    public:
        ...
        int insert_edge(int u, int v, int w);

        int delete_edge(int u, int v, int w);

        int _insert_edge(int u, int v, int w);
...
//=================== implementation ==================
    ...
    bool operator<(const Adj &x, const Adj &y) {
        if (x.w < y.w) return true;
        if (x.w > y.w) return false;
        return x.v < y.v;
    }
    ...
    int DynamicMST::insert_edge(int u, int v, int w) {
        if (!insert_edge_in_graph(u, v, w)) return -1;
        return _insert_edge(u, v, w);
    }
    ...
    int DynamicMST::_insert_edge(int u, int v, int w) {
    ...
    void DynamicMST::search_min_e(vector<int> &cand, Edge e_old) {
        ...
        if (rst == 2) {
            s1 += l.size();
            s2 += nodes[cand[cand.size() - 1]].sub_cnt;
            ++t;
        }
    }
    ...
    int DynamicMST::update_min_e(int u, int v, int w) {
        ...
        for (int fu = u, fv = v; fu != fv;)
            if (nodes[fu].sub_cnt < nodes[fv].sub_cnt) {
                if (eu < nodes[fu].min_e) {
                    nodes[fu].min_e = eu;
                    ++cnt;
                }
                fu = nodes[fu].p.v;
            } else {
                if (ev < nodes[fv].min_e) {
                    nodes[fv].min_e = ev;
                    ++cnt;
                }
                fv = nodes[fv].p.v;
